...
TEST_F(MarkerConversionTest, MakeMarkerFromPolygonLineStrip)
{
  ...
  EXPECT_EQ(arr.markers[0].points.size(), 4u);
...
TEST_F(MarkerConversionTest, CreatePullOverAreaMarkerArray)
{
  ...
  for (size_t i = 0; i < ring.size(); ++i) {
    EXPECT_DOUBLE_EQ(pts[i].x, ring[i].x());
    EXPECT_DOUBLE_EQ(pts[i].y, ring[i].y());
    EXPECT_DOUBLE_EQ(pts[i].z, z);
  }
}
...
TEST_F(MarkerConversionTest, CreateObjectsMarkerArray)
{
